            ...
            <div class="w-full bg-gray-100 pl-64 max-[1200px]:pl-0 min-h-screen relative" id="main-content">
                ...
                <div class="p-6 flex flex-wrap justify-between">
                    <section class="w-[60%] shadow-2xl p-6 max-[1300px]:w-[100%]">
                        <div class="flex justify-between max-[500px]:flex-col max-[500px]:text-center">
                            <h4 class="mt-4">Personal Information</h4>
                            <button @click="profileStore.showUpdatePassModal = true" class="max-[500px]:mx-auto mt-4 ml-2 relative btn btn-success w-[117px] h-[40px]"><LockClosedIcon class="h-5 w-5 text-white absolute top-[9px]" /> <span class="block text-right">Password</span></button>
                        </div>
                        <form class="mx-auto rounded-[12px] p-[25px] relative">
                            <div class="flex flex-wrap justify-between">
                                <div class="relative w-[48%] max-[600px]:w-full">
                            ...
                            <button @click.prevent="profileStore.updateProfileHandler" class="d-block p-2 bg-[#198754] hover:bg-[#177549] text-center text-[#fff] mt-5 rounded-lg px-4" :class="{ 'opacity-[0.5]': profileStore.isUpdatingInfo, 'pointer-events-none': profileStore.isUpdatingInfo }">{{ profileStore.isUpdatingInfo ? 'Updating...' : 'Update' }}</button>
                        </form>
                    </section>
                    <section class="w-[39%] max-[1300px]:w-[100%]">
                        <div class="mb-2 shadow-2xl p-12 text-center">
                            ...
                            <div v-else>
                                <img v-if="newPicture" :class="{ 'opacity-[0.5]': profileStore.isUpdatingImg }" class="object-cover w-[200px] h-[200px] max-w-[100%] block rounded-[50%] mx-auto" :src="profileStore.apiUrl + `uploads/${newPicture}`" alt="user">
                                <img v-else :class="{ 'opacity-[0.5]': profileStore.isUpdatingImg }" class="object-cover w-[200px] max-w-[100%] h-[200px] block rounded-[50%] mx-auto" :src="profileStore.apiUrl + 'uploads/default_photo.jpg'" alt="user">
